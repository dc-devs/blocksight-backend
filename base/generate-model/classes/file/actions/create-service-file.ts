...
const createServiceFile = ({ filePaths, modelName, attributes }: IProps) => {
	const data = `import { Prisma } from '@prisma/client';
import { Injectable } from '@nestjs/common';
import { ${modelName.singular.pascalCase} } from './dto/models/${
		modelName.singular.camelCase
	}.model';
...
const select = ${JSON.stringify(attributes)};

@Injectable()
export class ${modelName.plural.pascalCase}Service {
	constructor(private prisma: PrismaService) {}

	findAll(findAll${modelName.plural.pascalCase}Input: FindAll${
		modelName.plural.pascalCase
	}Input): Promise<${modelName.singular.pascalCase}[]> {
		const { skip, cursor, take, orderBy, where } = findAll${
			modelName.plural.pascalCase
		}Input;
	...
	async findOne(
		findOne${modelName.singular.pascalCase}Input: FindOne${
	...
	create(create${modelName.singular.pascalCase}Input: Create${
		modelName.singular.pascalCase
	}Input): Promise<${modelName.singular.pascalCase}> {
		const data = create${modelName.singular.pascalCase}Input as Prisma.${
		modelName.singular.pascalCase
	}CreateInput;
